...
export default defineStore('chatStore', {
	...
	actions: {
		...
		readedMessage(pos) {
			...
			if (!chat) return;
			// 已读状态偶尔异常，目前找不到原因，暂时通过回溯更新1000条的方式解决
			let idx = Math.max(0, chat.readedMessageIdx - 1000);
		...
		insertMessage(msgInfo, chatInfo) {
			...
			if (msgInfo.type == MESSAGE_TYPE.IMAGE) {
				...
				chat.lastContent = "[动画表情] " + JSON.parse(msgInfo.content).name;
			...
			} else if (messageType.isNormal(msgInfo.type)) {
				chat.lastContent = "[暂不支持该消息类型]"
			}
		...
		recallMessage(msgInfo, chatInfo) {
			...
			for (let idx in chat.messages) {
				let m = chat.messages[idx];
				if (m.id && m.id == id) {
					// 群聊中管理员撤回消息不显示提示，私聊和自己撤回时显示提示
					if (chat.type == 'PRIVATE' || !msgInfo.adminRecall) {
						// 改造成一条提示消息
